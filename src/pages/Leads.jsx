...
import { leadsService, clientesService, ascensoresService, tiposServicioService, usuariosService } from '../services';
...
import { ESTADOS_LEAD, ESTADO_LEAD_INGRESADO } from '../utils/estadoLead.js';

const inicial = { nombre_contacto: '', telefono: '', canal: 'WhatsApp', id_tipo_servicio_solicitado: '', cliente_existente: false, id_cliente: '', id_vendedor: '', observaciones: '' };
...
export default function Leads() {
  ...
  useEffect(() => {
    Promise.all([clientesService.list(), ascensoresService.list(), tiposServicioService.list(), usuariosService.list()])
      .then(([c, a, t, u]) => { setClientes(c); setAscensores(a); setTipos(t); setUsuarios(u); })
      .catch(() => {});
  }, []);
    ...
    <>
                  ...
                  <tr key={l.id} className="table-row-hover">
                    ...
                    <td className="table-td text-right">
                      <div className="flex flex-wrap gap-2 justify-end text-xs">
                        {puedeCotizar && l.estado_lead !== ESTADO_LEAD_INGRESADO && l.id_cliente && (
                          <button
                        ...
                        {puedeConvertir && l.estado_lead !== ESTADO_LEAD_INGRESADO && (
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Nuevo lead"
        ...
        <form onSubmit={guardar} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div><label className="label">Vendedor</label><select className="select" value={form.id_vendedor} onChange={e => setForm(f => ({ ...f, id_vendedor: e.target.value }))}><option value="">—</option>{usuarios.map(u => <option key={u.id} value={u.id}>{u.nombres}</option>)}</select></div>
